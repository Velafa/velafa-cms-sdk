...
When a site has **only one** locale, Atlas publish writes `sitemap.xml` / `llms.txt` / `llms-full.txt` **without** a locale path prefix (e.g. `https://example.com/about`, not `/en-mt/about`). With two or more locales, published artifact URLs are prefixed (`/en-mt/about`, `/nl-nl/about`, …).
